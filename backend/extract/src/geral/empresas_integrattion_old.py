...
class extractGeempre():
    ...
    def process(self):
        try:
            ...
            companiesExcel = leXls_Xlsx('C:/Programming/baymax/backend/extract/data/empresas_integrattion_old.xlsx', 'Já Avaliadas - Hist')
            ...
            for companie in companiesExcel:
                ...
                responsibleFinancialClientOld = funcoesUteis.treatTextField(companie[4])
                obsAccountPaidOld = funcoesUteis.treatTextField(companie[8])
                dateAccountPaidOld = funcoesUteis.treatTextField(companie[7])
                layoutsAccountPaidOld = funcoesUteis.treatTextField(companie[9])
                hasIntegrated = funcoesUteis.treatTextField(companie[10])

                dataCompanie = {
                    'codi_emp': codi_emp,
                    'responsibleFinancialClientOld': responsibleFinancialClientOld,
