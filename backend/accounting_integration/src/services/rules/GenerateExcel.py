...
class GenerateExcel(object):
    ...
    def sheetPayments(self, payments):
        ...
        sheet.set_column(10,10,options={'hidden':True})
        sheet.set_column(11,11,options={'hidden':True})
        ...
        sheet.write(0, 10, "Vencimento", self._cell_format_header)
        sheet.write(0, 11, "Emissão", self._cell_format_header)
        sheet.write(0, 12, "Valor Pago", self._cell_format_header)
        sheet.write(0, 13, "Desconto", self._cell_format_header)
        sheet.write(0, 14, "Juros", self._cell_format_header)
        sheet.write(0, 15, "Multa", self._cell_format_header)
        ...
        sheet.write(0, 19, "Historico Financeiro", self._cell_format_header)
        sheet.write(0, 20, "Categoria", self._cell_format_header)
        sheet.write(0, 21, "Plano de Contas", self._cell_format_header)
        sheet.write(0, 22, "CNPJ Pagador", self._cell_format_header)
        sheet.write(0, 23, "Historico Extrato Bancário", self._cell_format_header)
        sheet.write(0, 24, "Conta Contabil Sistema Cliente", self._cell_format_header)

        # ordena os payments por data
        payments = sorted(payments, key=itemgetter('bank', 'paymentDate'))
        ...
        for key, payment in enumerate(payments):
            ...
            sheet.write(row, 10, dueDate, self._cell_format_date)
            sheet.write(row, 11, issueDate, self._cell_format_date)
            ...
            sheet.write(row, 22, cgcePaying)
            sheet.write(row, 23, historicExtract)
            sheet.write(row, 24, accountCodeOld)
